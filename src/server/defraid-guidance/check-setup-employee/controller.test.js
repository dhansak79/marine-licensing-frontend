...
describe('#defraIdGuidanceCheckSetupEmployeeSubmitController', () => {
  ...
  test('should redirect to register new org page if user needs to create a new account', async () => {
    ...
    expect(headers.location).toBe(routes.defraIdGuidance.REGISTER_NEW_ORG)
  })
  ...
  test('should redirect if user needs to be added to an existing organisation', async () => {
    ...
    expect(headers.location).toBe(routes.defraIdGuidance.ADD_TO_ORG_ACCOUNT)
  })
})
